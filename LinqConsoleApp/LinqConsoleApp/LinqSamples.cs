...
namespace LinqConsoleApp
{
    public class LinqSamples
    {
        ...
        */
        ...
        {
            var maxS = (from emp in Emps select emp.Salary).Max();
            var res = Emps.Where(emp => emp.Salary == maxS);
            foreach (var re in res)
            {
                Console.WriteLine(re);
            }
        ...
        {
            var res = Emps.GroupBy(emp => emp.Job).Select(x => new
            {
                Praca = x.Key,
        ...
        {
            var res = Emps.Where(emp => emp.Job == "Frontend programmer").OrderByDescending(emp => emp.HireDate).Take(1).Select(x => new
            {
                Id = x.Empno,
                Nazwisko = x.Ename,
                Praca = x.Job,
                Pensja = x.Salary,
                DataZatrud = x.HireDate,
                DepartID = x.Deptno
